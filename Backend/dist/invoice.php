...
$sql = "SELECT invoice_id, user.name AS first_name, last_name, email, ship_address_line1, ship_address_line2, order_date  
FROM invoice 
...
<body>
    ...
    <div id="invoice">
        <div class="invoice mx-auto overflow-auto">
                ...
                <main>
                    ...
                    <table border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tbody>
                            ...
                            <?php
                            $user_id = $_SESSION['id'];
                            $sql = "SELECT invoice_id, user.name AS first_name, last_name, email, ship_address_line1, ship_address_line2, order_date,  watch.name, brand, description, details.quantity, watch.price, details.quantity * watch.price AS total_price_per_item_quantity
                            FROM invoice 
                            JOIN orders ON invoice.order_id = orders.order_id
                            JOIN user ON user.id = orders.user_id
                            JOIN shipping ON shipping.order_id = orders.order_id
                            JOIN details ON details.order_id = orders.order_id
                            JOIN watch ON watch.id = details.watch_id
                            WHERE invoice.invoice_id = $invoice_id";
            ...
            <footer>
                <button class="btn btn-lg mt-5 btn-order"><a href="user.php">Return to user</a></button>           
            </footer>
        </div>
